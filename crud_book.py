...
def create_book(db:Session,user:schemas.CreateBook):
	...
	# 	raise HTTPException(status_code=403, detail="add only 0 to 20 books ")
		...
		book = models.Book(Title = user.Title.lower(), Author = user.Author.lower(), Quantity = user.Quantity, Total_Quantity = user.Quantity, Created_At = datetime.now())
		db.add(book)
		db.commit()
		db.refresh(book)
	return book
...
def search_book(db:Session, title:str):
    db_book = db.query(models.Book).filter(models.Book.Title.ilike(f"%{title.lower()}%")).all()
